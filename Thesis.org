...
\begin{alignat*}{3}
  ...
  &\blank \concat_n \overline{s} \defeq \term{ex-falso} &&: P_B^0(g s) \to P_A^0(f s) \\
  &D^0 \defeq \star &&: \1.
\end{alignat*}
...
The maps $t_B^n$ and $t_A^n$ are defined by induction on $n$. In the zero case, the domain of $t_B^0(b)$ is the empty type, so it is defined by $\term{ex-falso}$. The domain of $t_A^0(a)$ is the identity type $(a_0 = a)$, and inducting on it gives us the goal of inhabiting $Q_{\Sigma A}(a_0, \iota_A^0(\refl))$, which we can do by the basepoint $q_0$. In the successor case $n + 1$, we eliminate from the pushouts $P_B^{n + 1}(b)$ and $P_A^{n + 1}(a)$, so we use the dependent universal property of pushouts. This means that we need to give definitions of the maps on point constructors and a coherence over the path constructor. Let us begin with the point constructor cases.

To define $t_B^{n + 1}$, the behavior on point constructors consists of maps
\begin{alignat*}{2}
  &t_B^{n + 1}(b, \incl_B^n(\blank)) &&: (p : P_B^n(b)) \to Q_{\Sigma B}(b, \iota_B^{n + 1}(\incl_B^n p)) \\
  &t_B^{n + 1}(g s, \blank \concat_n s) &&: (p : P_A^n(f s)) \to Q_{\Sigma B}(g s, \iota_B^{n + 1}(p \concat_n s)).
\end{alignat*}
The statement of the second function is in a form after inducting on the term \linebreak $r : b = g s$ in the context. The maps are defined by
\begin{align*}
  &t_B^{n + 1}(b, \incl_B^n(p)) \defeq \tr_{Q_{\Sigma B}(b)}(\kappa_B^n(p), t_B^n(b, p)) \\
  &t_B^{n + 1}(g s, p \concat_n s) \defeq \tr_{Q_{\Sigma B}(g s)}(C^n(s, p), Q_{\Sigma S}(s, \iota_A^0(p), a, t_A^n(a, p))).
\end{align*}

Similarly for the definition of $t_A^{n + 1}$ on point constructors, we need to give
\begin{alignat*}{2}
  &t_A^{n + 1}(a, \incl_A^n(\blank)) &&: (p : P_A^n(a)) \to Q_{\Sigma A}(a, \iota_A^{n + 1}(\incl_A^n p)) \\
  &t_A^{n + 1}(f s, \blank \concat_{n + 1} \overline{s}) &&: (p : P_B^{n + 1}(g s)) \to Q_{\Sigma A}(f s, \iota_A^{n + 1}(p \concat_{n + 1} \overline{s})).
\end{alignat*}
The behavior on $\incl_A^n$ is analogous to the one of $t_B^{n + 1}$, specifically
\begin{displaymath}
  t_A^{n + 1}(a, \incl_A^n(p)) \defeq \tr_{Q_{\Sigma A}(a)}(\kappa_A^n(p), t_A^n(a, p)).
\end{displaymath}
...
\begin{displaymath}
  t_A^{n + 1}(f s, p \concat_{n + 1} \overline{s}) \defeq (Q_{\Sigma S}(s, \iota_A^{n + 1}(p \concat_{n + 1} \overline{s})))^{-1}(\Phi^n(s, p, t_B^{n + 1}(g s, p))).
\end{displaymath}
...
#+begin_conj
A family of coherences $K_S$ exists.
#+end_conj
...
- improve the definition of $t_A$ and $t_B$ by removing reconstructible data, similarly to how we removed $\blank \concat_n s$ from the zigzag construction data
...
* Footnotes
[fn:2] Since the types $A$, $B$ and $S$ can all live in different universes, we also formally have to raise the identity type and the empty type to their smallest universe containing all of them. This is done in the formalization, but it is omitted from the thesis text. 
