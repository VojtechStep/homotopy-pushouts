...
The standard coequalizer of $\D$ may be obtained as the pushout of $\spanDoubleArrow(\D)$. The pushout thus constructed can be visualized as consisting of a copy of $B$, a copy of $A$, and for every point $a : A$ there will be a path from $f(a)$ to $a$ and to $g(a)$, which corresponds to having a copy of $B$ with paths connecting every $f(a)$ to $g(a)$.
