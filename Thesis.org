...
#+name: thm:based-J
#+begin_thm
Consider a type $A$ and its element $a_0 : A$. Given a type family $P : (a : A) \to (a_0 = a) \to \UU$, there is an function
\begin{displaymath}
...
Identifications of elements $x, y : A$ also induce a map between fibers for any type family $P : A \to \UU$. This is called the *transport map*
\begin{displaymath}
  \tr_P : (x = y) \to P(x) \to P(y).
\end{displaymath}
...
\end{tikzcd}
#+end_center
We will occasionally mention *commuting cubes*, which \TODO.
...
#+begin_corol
\TODO[point preserving equivalences are contractible]
#+end_corol
